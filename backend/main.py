...
trusted_origins = [
    ...
    # лучше разрешить все chrome-extension origins через allow_origin_regex (по желанию),
...
async def analyze_request(request: AnalysisRequest):
    ...
    # --- 1) Анализ текста и поиск заголовков ---
    if text:
    ...
    if url:
        ...
        reg_term_res = url_analyzer.check_short_registration_period(url)  # ДОБАВЛЕНО
        ...
        results.extend([
            reg_age_res, reg_term_res, lexical_res, typesquat_res, subdomain_res,
            tld_res, protocol_res, domain_pattern_res, host_port_res, path_res, tls_res
        ])
        ...
        features_for_heuristic = {
            ...
            'short_registration_period': reg_term_res.is_suspicious,  # ДОБАВЛЕНО
            ...
            # ВАЖНО: no_hsts учитываем только для HTTPS
            'no_hsts': (is_https and hsts_res.is_suspicious),
    ...
    # --- 4) Признаки заголовков для модели ---
